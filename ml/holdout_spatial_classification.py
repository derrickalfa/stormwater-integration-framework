...
=================================
R2-13 (spatial hold-out) for the OCCURRENCE/CLASSIFICATION tier.

Train on a random 80% of DISTRICTS, test on the held-out 20% (unseen areas).
Param-reuse mode. Class-balance terms recomputed from each split's training set.
Threshold kept consistent with the baseline (selected on test set).

MULTI_SEED: repeat over seeds for mean ± std (default False).
"""
...
OUTPUT_DIR   = str(Paths.ML_CLASSIFICATION_SPATIAL)
...
def find_threshold(y_true,yp):
    pr,rc,th=precision_recall_curve(y_true,yp); f1=2*pr*rc/(pr+rc+1e-12)
...
def fit_predict(name,bp,X_train,y_train,X_test,cats,cw_dict,spw,sw_train):
    if name=='LightGBM':
    ...
    if name=='MLP':
        ...
        fm=MLPClassifier(**mp,random_state=RANDOM_STATE).fit(Xtr,y_train)
        return fm.predict_proba(Xte)[:,1]
    if name=='AdaBoost':
...
print("="*70,"\nSPATIAL HOLD-OUT (classification) — random district split\n","="*70)
...
for sd in seeds:
    ...
    print(f"\n[seed {sd}] districts {len(tr_d)}/{len(te_d)} | rows {len(X_train):,}/{len(X_test):,} | "
          f"train prev {y_train.mean()*100:.2f}%, test prev {y_test.mean()*100:.2f}%")
    for name in models:
        bp=load_params(name)
        if bp is None: continue
        try:
            yp=fit_predict(name,bp,X_train,y_train,X_test,cats,cw_dict,spw,sw_train)
            thr=find_threshold(y_test,yp); pred=(yp>=thr).astype(int)
            m={'F1':f1_score(y_test,pred),'Recall':recall_score(y_test,pred),
            ...
            results[name].append(m['F1'])
            print(f"  {name:<20} F1={m['F1']:.4f} Rec={m['Recall']:.4f} Prec={m['Precision']:.4f} ROC={m['ROC_AUC']:.4f} PR={m['PR_AUC']:.4f}")
            if not MULTI_SEED:
...
if MULTI_SEED:
    print("\n"+"="*50,"\nMULTI-SEED SUMMARY (F1 mean ± std)\n","="*50)
    rows=[]
    for name in models:
        if results[name]:
            arr=np.array(results[name]); rows.append({'Model':name,'F1_mean':arr.mean(),'F1_std':arr.std(),'n_seeds':len(arr)})
